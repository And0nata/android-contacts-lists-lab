...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String [] filterList = ContactType.getContactTypes();  // создайте массив строк. Для инициализации используйте метод ContactType.getContactTypes(),
        // который должен возвращать список всех доступных источников контактов

        ArrayList <FilterContactTypeUi> filterTypeUI= new ArrayList<>();// создайте список типа FilterContactTypeUi и заполните его с помощью цикла forEach
        for (String type : filterList) {
            filterTypeUI.add(createFilterContactType(type));
        } // forEach должен бежать по строковому массиву, который вы создали ранее
        filterTypeUI.add(0, createAllSelectedItem(filterList));
        filterContactTypesLiveDate.setValue(filterTypeUI);// вызовите меотод setValue() у переменной filterContactTypesLiveDate и передайте в качестве аргументы ваш список типа FilterContactTypeUi
    }
